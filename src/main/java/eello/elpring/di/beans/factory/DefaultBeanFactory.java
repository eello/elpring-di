...
public class DefaultBeanFactory implements LegacyBeanFactory {
	...
	public void registerBean(BeanDefinition def) throws
		...
		// 싱글톤 빈 등록
		singletonBeanRegistry.addSingleton(def.getBeanClassName(), bean);

		// 빈의 이름에 BeanDefinition 매핑
		beanDefinitionMapByName.put(def.getBeanClassName(), def);
	...
	private Object createBean(BeanDefinition def) throws
		...
		for (int i = 0; i < dependsOn.length; i++) {
			...
			if (candidates == null || candidates.isEmpty()) {
				...
				message.append("'").append(def.getBeanClassName()).append("'")
			...
			if (candidates.size() == 1) {
				...
				args[i] = singletonBeanRegistry.getSingleton(candidates.getFirst().getBeanClassName());
			} else {
				// 후보 빈이 여러개인 경우
				for (BeanDefinition candidate : candidates) {
					if (candidate.isPrimary()) {
						// @Primary 가 적용된 빈을 주입
						args[i] = singletonBeanRegistry.getSingleton(candidate.getBeanClassName());
						break;
					}
				}
	...
	public <T> T[] getBean(Class<T> beanType) {
		...
		for (BeanDefinition beanDefinition : beanDefinitions) {
			find.add(getBean(beanDefinition.getBeanClassName(), beanType));
		}
	...
	public boolean isRegistered(BeanDefinition def) {
		return isRegistered(def.getBeanClassName());
	}
